...
- **Phase R0, the modified-order math doesn't itemize on 04D/04E**: 06B itemizes the modification (card 1 `$200` = Hem + added Sleeve, added third garment `$80` → Subtotal `$360`), but 04D/04E's cards still read `$120/$80/$80` (= $280) while billing `$340` due / `$350` total. Seed totals carry 360/20; the cards render the frames verbatim. Kevin is updating 06B to `$350` in Figma — re-check when the frame changes.
- **The appointment date is Jul 12 (Kevin, Phase R0.1)** — per the 09 frame (`JUL 12 / Appt Date: Jul 12, 7PM`); the seed carries it. 01/01a's frames still show the stale `AUG 29` card (text-parity ALLOW; their diff baselines absorb it until re-synced). 04C/05's tailor cards still say `Thu, Jul 9` while 04D says pinned `Thu, Jul 12` — still open.
- **02B's frame backdrop is stale (Kevin-approved)**: 538:1382 still draws the pre-R0 02 (Address pill, old header) behind its scrim; per Kevin the build shows the live `view02()`. Its diff baseline and text-parity ALLOW entries absorb it until the frame is re-synced.
...
- **The deposit row always reads `-$20` (Kevin, Phase R0.1)** — the 04D/06A/06B frames still write it spaced (`- $20`); built unspaced everywhere (text-parity ALLOW covers the spaced frame nodes).
