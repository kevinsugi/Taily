...
- `T2 / Chat Bubble` master text is 14px — was off the scale until Aug 2026, when `size/14` became a real Figma variable (the Selector menu binds to it); `.bubble` and `.selector__option` now use `--size-14`. Photo-tile gradient starts at `#E2DCD1`, one digit off `neutral-200`. The 02a sheet draws a 40×4 grabber where 04a/04b use 36×4. Both still await Kevin's call.
- **Selector master vs 02 instances disagree**: the master's Job trigger has a 12px chevron and reads "Hem / Adjust Length"; the 02 frame's card instances hide the chevron and read **"Hem / Adjust"** — which is not a v3 `JOB_TYPES` key (added to `data.js` at $100 so the two seeded cards keep the frame's "$20 Deposit"). The master's Job menu also lists short names (Taper / Sleeve / Resize / Repair) where the build's menus use the real `JOB_TYPES` keys. Raised, not silently fixed.
