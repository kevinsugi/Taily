...
Phase R2 screens: `r1-reschedule-popup` (558:1867, modal over 05 — opened from 04C's / 05's Reschedule / Cancel; confirming runs `cancelAppointment()` → 05X), `05x-appointment-cancelled` (558:3817), `pv3-photo-viewer` (559:2112, filmstrip over 04D — opened by tapping any Before/Pinned photo row via `wirePhotoViewer()`; pre-appointment ViewOnly tiles are deliberately inert), `rc1-request-changes` (559:3265, modal over 05 per the frame — opened from Request Changes on 06A/06B; Sounds Good dismisses). Live popups mount through `modalOverlay()` (components.js); the registered routes render the frame-verbatim backdrop (`view05` / `view04d`) for the diff harness.
