...
## Round 16 — Figma review: Concierge fee, Fee Row master, calendar picker, delivery-only handoff, decline re-match (Sep 21 2026; Figma synced)

### Kevin's asks (source-of-truth frames / masters)
- Garment cards (User - Garment Card 57:197): the reduced palette — everything ink, Before | Pinned tabs.
- Visitation fee → **Concierge fee** on every screen; Taily keeps it (the tailor never sees it); caption "Covers pickup, fitting, and delivery."; three variations $50 / $90 (5+ items) / $140 (9+ items).
- Fee rows: the new **Fee Row** master (745:5889, Default / Modified) everywhere.
- Alterations (est.): the subheading "Final price determined at appointment" (answer 12 settled the strings).
- Heading with Back: the Back in the same group as the pill + heading; explanations in their own row at the screen padding.
- Garment Tile (252:1235): no gradient; the remove tap zone is the bottom 25% of the tile.
- Date & time: the **Calendar** master (768:7277 over Calendar / Cell 764:4597) replaces the wheel sheet.
- Cancelled states: the Declined state is gone — a decline looks for a new tailor.
- Delivery only: no pickup; 05 / 05B revised; new 03 - Order Status / Delivery Scheduled (775:5513) for checking on a scheduled delivery.

### Answers (38 questions; Kevin: defaults except where noted)
1 leftover per-service price on the Default variant → not built · 2 inactive "Pinned" tab **neutral-400** · 3 "Concierge fee" (lowercase f, the frames) · 4 tier + status in one title, **hyphens everywhere** ("Concierge fee (5+ items) - Paid 7/7/26") · 5 bands 1–4 / 5–8 / 9+ · 6 no-show protection = **a flat $25 paid by Taily** · 7 the tailor frames' Visitation rows hidden + payouts retyped on sync · 8 Expired reads "Concierge fee - Refunded" · 9 captions ink 12 Medium (the master) · 10 the Modified variant only on rows that changed at the visit (the frames' green rows were a slip) · 11 one caption rule: pre-visit rows caption all three lines, post-visit Alterations ("Finalized at the appointment.") + the fee, refunded rows none · 12 **"Price is finalized at the appointment." pre-visit, "Finalized at the appointment." post-visit** · 13 the heading column starts **28px** in (Back 23 + gap 5, every heading row) · 14 06 has no Back and no pill · 15 03 / Requested's hero keeps shrinking · 16 top 25% adds, bottom 25% removes, the middle is inert · 17 1px border, Selected_One's neutral-700 label as drawn · 18 the wheel swaps in place inside the popup; the scrim closes; a pickup day without a time shows the Error variant once · 19 **rush ladder** by days after the visit: +$150 / +$100 / +$50, four or more free (replaces the flat $150 within 24 h) · 20 caption "Need-by is N day(s) after your visit." · 21 the need-by is never the visit day · 22 **delivery can be today**: today ink, the choice outlined, the days between shaded; a delivery chosen for today draws today outlined · 23 the wheel everywhere (Time / Chip unused) · 24 the Address row pre-fills the visit address and opens the 02.2 sheet · 25 month arrows page months, past days muted · 26 05's error reads "Please select a delivery time" · 27 a pill on today's date reads **"Today · 7:00 PM"** · 28 a decline re-matches silently (the Declined route / fixture / flow removed; T03A stays) · 29 03 / Cancelled's pill stays Cancelled (the frame's "Declined" flagged) · 30 Expired stays; **a tailor has 12 hours to respond**; 03 / Requested's copy is Kevin's ("Most matches happen within the same day…") · 31 **05 → 05.1 Confirm Delivery → 05.2 Delivery Confirmed**, 03 / Delivery Scheduled from Bookings / Home · 32 delivery is inside the concierge fee (no $20 row; "Rush Delivery" not built) · 33 no CTA on 03 / Delivery Scheduled · 34 T07 copy: "Sarah scheduled delivery for … A Taily courier will collect the items — your payout is released on handoff." · 35 09's ready card CTA "Schedule Delivery" · 36 05's body names the amount · 37 the app's date grammar everywhere ("Thu, Jul 23 · 5:00 PM"; 06's "Paid at delivery" = the delivery date) · 38 Figma synced.

### Results
- **Money (data.js)**: `FEE_LABEL` "Concierge fee", tiers `VISIT_FEE_TIERS` $50 / $90 / $140 with `min` 1 / 5 / 9 (`feeTierMin`, `feeTierLabel` → "Concierge fee (5+ items)"); `tailorFee` / `TAILOR_FEE_TIERS` / `DELIVERY_FEE` gone — `payoutParts` = the alteration prices, `NO_SHOW_COMP = 25` (`noShowComp()`); rush = `RUSH_LADDER [150, 100, 50]`, `rushDays(when, needBy)` (calendar days), `rushFeeForDays`, `rushFee`, `rushCaption(days)`; captions `VISIT_FEE_CAPTION` "Covers pickup, fitting, and delivery.", `ALTERATIONS_EST_CAPTION`, `ALTERATIONS_FINAL_CAPTION`, `HANDOFF_CAPTION` "Alterations are paid at delivery.". `scripts/money.mjs` follows (FEE t3 = 9 items, COMP flat, DELIVERY 0, `RUSH_LADDER_STEPS`, `rushCap`, `CAPTION.final`, `feeLabel`).
- **Fee rows**: `feeRow` CSS = the master (68px Bold 16 amount cell, Medium 16 ink title, Medium 12 ink description, 12 / 8 padding, `--changed` success); `orderRows(t, { est, feeDesc, due, info, tier, total, captions, feeCaption })` — `feeDesc` is the status suffix ("Due Today" / "Paid 7/7/26" / "Refunded" / "Kept") appended to `feeLabelFor(t)`; the caption rule lives here (`captions` defaults to `est`); a re-tiered fee's caption is `feeTierNote(t)` (the explanation sentence — no separate note paragraph); `receiptRows` = Alterations (final caption) / Concierge fee - Paid <date> / [Rush fee] / Total / Paid at delivery <date>. "Due at delivery" everywhere. 03 / Confirmed and Reminder read "Due Today" until the customer confirms (`feeLocked`), then "Paid <date>".
- **Heading rows**: `.heading-row` gap 5 (the column at 28); `statusHero` wraps body + extra in `<!--notes--><div class="status-hero__notes">…</div><!--/notes-->` and `headingRow()` lifts that block out of the Back row so explanations sit at the padding (04 wraps its own paragraphs the same way).
- **Garment Tile**: white fill, 1px neutral-700 border, zones = 25% of the tile (`.garment-tile__zone { height: 25% }`); the `--tile-gradient` token is unused. Pinned tab inactive = neutral-400.
- **Calendar** (`js/calendar.js`, CSS in components.css): `calendar({ mode, month, selected, anchor, anchorTime, min, time, address, errors, white })` + `wireCalendar(root, { onDay, onMonth, onTime, onAddress })`; five or six week rows as the month needs (`monthGrid`); cell states `.cal-cell--default/muted/weekday/selected/between/needby` (+ `__sub` for "7:00PM" / "Today" / "+$150"); the rush ladder is drawn on the days after the visit from `RUSH_LADDER`; the time picker = `pickerColumns(time)` on the shared `wheelScroll` (9 AM – 9 PM, :00 / :30; the master draws 36px rows, the wheel 40 — raised). Icons `ICON_CAL_CLOCK / PIN / PREV / NEXT` (icons.js).
- **02 / 02.1**: `openCalendarOverlay(mode)` (`openDateTimeOverlay('appt' | 'needby')` keeps the name) opens the calendar as a modal popup at 18 / 215 (`.calendar-popup`); a day tap stores it at once (`state.appt.when` / `needBy`, "Jul 12, 7:00 PM" / "Jul 17"), "Select Time" swaps in the `.time-picker`, its settle writes the time; the scrim: picker → calendar; pickup day without a time → the Error line once; else close. `pillText()` — "Today · 7:00 PM" on today's date. The tailor's T03A proposal still uses the wheel sheet (`openDateTimeOverlay('custom', …)` → `openWheelSheet`). Route 02.1 = the frame (756:7291 replaces 277:2916 in screens.json).
- **Delivery**: 05 rebuilt (`viewItemsReady`, the Delivery calendar inline, white; `state.ui.delivery` draft { day "Jul 23", time, address, month, errors }; Confirm → validate → `openConfirmDelivery` (05.1, new route `05.1-confirm-delivery` 775:4991) → `chooseFulfilment('delivery', window, date, a, address)` → `openDeliveryConfirmed` (05.2, route `05.2-delivery-confirmed` = frame 609:2779, was 05.1-window-confirmed-dated) → Done → `03-status-delivery-scheduled` (775:5513; `visitBlock(rows, { title: 'Delivery Details' })`). `statusScreen(a)` → that screen for a ready order with a window. `deliver()` stamps `deliveredAt` = the window label; 06 draws "Delivered: Thu, Jul 23 · 5:00 PM" as its third visit row, no Back / pill, and the $280 two-card order on a cold load. 01 / 03-Tailoring: "Schedule Delivery" / "Change Delivery". Routes 05a / 05b / 05.1-window-confirmed removed (frames gone). `openAddressOverlay(onSaved)`.
- **Decline**: `declineAppointment(a)` puts the request back to matching (`declinedBy` remembers the tailor; `assignTailor` skips them → Jordan); `TERMINAL_STATUSES` = cancelled / expired; the tailor side reads `declinedBy.includes('marco')` as 'declined' (`canon`), so T01 drops it. 03 / Cancelled keeps its Cancelled pill. `EXPIRY_MINUTES` = 720.
- **Tailor**: `payoutRows({ payout, rush })` — no Visitation row; `orderMoney` / `jobView` without `visitCut`; T08 without the fee price row; `noShowCompOf` flat; T07 / T07 Waiting copy.
- **Harness**: `npm run check` green — diff 61 screens (22 baselines ≥ 4% re-accepted, see Flags), text parity 61 / 61 (new ALLOW entries: the round-16 date grammar, the 05.2 / 06.1 backdrops, the tailor frames' pickup fiction, the Reminder callout comma, T01's fixture timer), click-throughs customer 248 / tailor 361 / sync 1080 / flows 91, hygiene clean (`--size-8` token for the calendar sublabel). The customer and sync runs book the visit on the calendar (day cell → wheel → scrim) with a need-by four days out (three is a $50 rush now) and walk 05 → 05.1 → 05.2 → 03 / Delivery Scheduled → 06; the rush probe covers the whole ladder; the decline probe checks the re-match (searching, `declinedBy`, Jordan accepts).
- **Figma (synced)**: user page — 65 Fee Row instances swapped Modified → Default (the changed rows on 04 Modified / Removed / Re-tiered kept), captions set per the rule (21 rows shown / retyped), "— Kept" → "- Kept", Expired → "- Refunded", "Due at handoff" → "Due at delivery", "Paid at delivery 7/17/26" → 7/23/26, the $20 Delivery rows (06.1 ×2, 05.2) and 03 / Summary's Rush Delivery row hidden with the totals retyped ($410 / $360), 35 heading rows gap 5, 03 / Cancelled pill → Cancelled, 09 "Schedule Delivery", date grammar (06, 03 / Delivery Scheduled, the 05 / 05.1 / 05.2 modals, 05's "5:00 PM"), 05.2's backdrop rebuilt from 05 (old body hidden); tailor page — 9 Visitation rows hidden, $225 → $200 / $385 → $360 / "Accept Request · $200" / "Payout $200 → $360 (+$160)", 7 heading rows gap 5, T07 copy; Components — Garment Tile + Calendar descriptions, Delivery_error "delivery time", the Appointment Card master's "Schedule Delivery"; second pass — 05.1 clipped, the explanation paragraphs moved under the Back row on 04 / Default, T08, the four T03B frames and the 06.1 backdrops, the 06.1 backdrops aligned to 06's $280 order. Refs re-exported, baselines re-accepted.

### Flags
- **Baselines re-accepted at 4–15%** (drift, not design): the cancelled family (03 / Cancelled 9.6, Expired 10.5, Tailor Cancelled 8.9, No-Show 8.8, Unconfirmed 8.8) sits one pixel off the frames after Kevin's edits (fractional frame heights — 1032.67 on 05.1, for one — rasterise on half pixels); T03B / Withdrawn 14.3, T07 8.5, T08 11.8: the tailor bodies wrap one line earlier than the frames under Chromium's metrics; T06 11.3 / T04 7.8 / T05 8.5 are the standing Appt-variant cards.
- **04 / Removed** (13.3%): the frame no longer draws the "Removed at the visit — Suit Jacket · Sleeve / Adjust Length  $80" row the state exists for; the code keeps it. Say if it should go.
- **05.1 - Confirm Delivery** exported 430 wide (the modal's shadow, the frame unclipped) — `clipsContent` set on sync; the frame's own height is fractional (1032.67).
- **Heading paragraphs**: Kevin's rule (explanations in their own row) was still unmet on 04 / Default, T08, the four T03B frames and the 06.1 backdrops — their paragraphs were moved under the Back row on sync (a "Notes" frame, gap 4). 02's address line stays in the column (Kevin).
- **03 / Cancelled's pill** read "Declined" — set to Cancelled on sync. **03 / Expired** read "Concierge fee — Kept" — set to "- Refunded" (nothing was charged); its body copy "Nothing was charged and the hold on your card is released." (Kevin's) replaced the code's em-dash line on every hold-released screen.
- **Addresses**: the user frames' Visit Details rows now append the zip ("◉ 88 Leonard Street, 10013", Regular 16 ink) while the tailor frames say "88 Leonard Street" — the code prints the appointment's `place` ("88 Leonard Street") on both sides (parity ALLOW). Say if the seed address should carry the zip.
- **03 / Reminder's callout**: the frame says "non-refundable, no-shows included" (comma) where every other frame and the code use " — no-shows included".
- **06.1 backdrops**: aligned to 06's $280 two-card order on sync (third card hidden, totals $280 / $330 / $280), but they still carry a Back row and a Need-by row that 06 no longer has (ALLOW'd; 06.1's own render follows 06).
- **Tailor fiction still says pickup** on T07's Return Visit rows ("Fri, Jul 17 · 3:00 PM / 1025 Broadway") and the T01 frames ("Ready for Pickup", "Pickup: Sept 1, 2PM"); the code's T07 fixture keeps those rows so the frame matches. Live jobs read the delivery window.
- **Masters**: the Fee Row master binds title / description / showDescription to Figma VARIABLES rather than component properties (instance overrides were written on the text nodes directly); the Calendar master is 354 wide inside a 350 body (as drawn) and its 308 Days grid overflows the 24 padding by 2px; the Time Picker master draws 36px rows where the shared wheel is 40px (the picker has no frame instance, so nothing diffs); Time / Chip is unused; 02's second card still has no service line (standing); 03 / Requested's hero still overflows its Back row (standing).
- **Decline is silent** by Kevin's choice — the customer only sees 03 / Requested again. A one-line notice ("Marco couldn't take it — finding another tailor") would be a small follow-up.
...
- (nothing pending — rounds 12–16 were synced in full)
