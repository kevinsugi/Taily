...
import { view08 } from './08-journey-complete.js';
...
export function openLeaveReview() {
  ...
  modalOverlay(sheetHtml({ sub: `${a.name ?? 'Marco Tailor'} · ${n} items · Sept 1`, live: true }),
    { dataS: '08c-leave-review' }, wireSheet);
}

/* Route registration keeps the frame-verbatim render for the diff
   harness: 08 as backdrop, scrim, fixture sheet. */
function renderScreen(s) {
  return `<div class="screen-sheet" data-s="08c-leave-review">
  <div class="sheet-backdrop" aria-hidden="true">${view08(s)}</div>
...
function wire(root) {
  wireSheet(root, () => go('08-journey-complete'));
}

register('08c-leave-review', renderScreen, wire);
