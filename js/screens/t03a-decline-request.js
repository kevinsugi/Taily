...
function wire(root) {
  ...
  root.querySelectorAll('[data-reason]').forEach((el) => el.addEventListener('click', () => {
    tailorUi(state).declineReason = Number(el.dataset.reason);
  ...
  root.querySelector('[data-act="back"]')?.addEventListener('click', () => back() || go('t02-appointment-request'));
}
